    ...
    <AnimatePresence>
      {isVisible && (
        ...
        >
          ...
          <AnimatePresence>
            {showDiagnostic && (
              <motion.div
                ...
                } bottom-0 bg-gradient-to-br from-yellow-500 to-orange-500 text-white p-4 rounded-2xl shadow-2xl border border-yellow-400/30 max-w-xs`}
              >
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1">
                    <h4 className="font-bold text-sm mb-1">Que tal fazer um diagnóstico estratégico agora?</h4>
                    <p className="text-xs text-white/90 mb-3">Descubra como otimizar suas vendas B2B</p>
                    <div className="flex space-x-2">
                      <button
                        onClick={handleDiagnosticAccept}
                        className="bg-white text-orange-600 text-xs font-semibold px-3 py-1 rounded-lg hover:bg-white/90 transition-colors"
                      >
                        🚀 Fazer Diagnóstico
                      </button>
                      <button
                ...
                <div
                  className={`absolute bottom-2 ${
                    ...
                    position.includes('right')
                      ? 'border-l-[6px] border-l-orange-500 border-t-[6px] border-t-transparent border-b-[6px] border-b-transparent'
                      : 'border-r-[6px] border-r-orange-500 border-t-[6px] border-t-transparent border-b-[6px] border-b-transparent'
                  }`}
                />
              </motion.div>
            )}
          ...
          {/* Tooltip (só aparece se não estiver mostrando o balão de diagnóstico) */}
          <AnimatePresence>
            {isHovered && !showDiagnostic && (
              <motion.div
                ...
                className={`absolute ${
                  position.includes('right') ? 'right-16' : 'left-16'
                } top-1/2 transform -translate-y-1/2 bg-gray-900 text-white px-3 py-2 rounded-lg text-sm whitespace-nowrap shadow-lg border border-gray-700`}
              >
                ...
                <div
                  className={`absolute top-1/2 transform -translate-y-1/2 w-0 h-0 ${
                    position.includes('right')
                      ? 'right-[-6px] border-l-[6px] border-l-gray-900 border-t-[6px] border-t-transparent border-b-[6px] border-b-transparent'
                      : 'left-[-6px] border-r-[6px] border-r-gray-900 border-t-[6px] border-t-transparent border-b-[6px] border-b-transparent'
                  }`}
                />
              </motion.div>
            )}
          ...
          <motion.a
            ...
            whileTap={{ scale: 0.95 }}
            className="group relative flex items-center justify-center w-14 h-14 bg-green-500 hover:bg-green-600 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer"
